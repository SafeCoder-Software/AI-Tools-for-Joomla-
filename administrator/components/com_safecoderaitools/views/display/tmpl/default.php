        ...
        <?php foreach ($prompt->ChoiceList as $choice) : ?>
            ...
            <li class="list-group-item list-group-item-action flex-column align-items-start">
                <div class="d-flex w-100 justify-content-between mb-2">
                    ...
                    <?php endif; ?>
                    <small>
                        <a id="ChoiceCopy<?php echo $x; ?>" class="btn btn-primary btn-sm" href="javascript:void(0);" data-scs-id-copy-button="<?php echo addslashes(strip_tags($x)); ?>" onclick="copyText(event, <?php echo addslashes(strip_tags($x)); ?>, 0)">
...
<script type="text/javascript">
    Joomla.submitbutton = function(task) {
        ...
        if (task == 'display.Remove') {
            if (confirm("<?php echo addslashes(trim(strip_tags(Text::_('COM_SAFECODERAITOOLS_DELETE_MSG')))); ?>")) {
                Joomla.submitform(task);
            }
        } else if (task == 'display.RunAgain') {
            if (confirm("<?php echo addslashes(trim(strip_tags(Text::_('COM_SAFECODERAITOOLS_RUN_AGAIN_MSG')))); ?>")) {
                Joomla.submitform(task);
            }
        } else {
            Joomla.submitform(task);
